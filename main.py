...
website_url = 'https://crosspack.ru/?utm_source=tgbot'
...
first_message = f"<b>Друзья, добрый день!</b>\n\n" \
                ...
                f'''Данный чат создан для открытого диалога с основателем кухни <a href="{website_url}">Михайлик ''' \
                ...
                f'''смотрите на нашем <a href="{website_url}">сайте</a>)</i>\n\n''' \
...
def start(message):
    try:
        ...
        bot.send_message(message.from_user.id, "Hello", reply_markup=main_markup)
...
def button_response(message):
    try:
        ...
        if message.text == 'Правила чата':
            bot.send_message(message.from_user.id, first_message, parse_mode='html')
        if message.text == 'Частые вопросы':
            ...
            bot.send_message(message.from_user.id, "Выберите интересующий вас вопрос", reply_markup=user_markup)
        if message.text == 'Перейти на сайт':
            bot.send_message(message.from_user.id, f'''Переходите на наш сайт по <a href="{website_url}">ссылке</a>''', parse_mode='html')
        if message.text == 'Связь с оператором':
            bot.send_message(message.from_user.id,
                             f'''Нажми <a href="https://api.whatsapp.com/send/?phone=79295191725&text&app_absent=0">сюда</a> и попадешь в чат с оператором''', parse_mode='html')
        if message.text == 'КАК ДОЛГО ХРАНИТСЯ ЕДА Михайлик Kitchen?':
            bot.send_message(message.from_user.id, "При соблюдении температурного режима, который указан на упаковке. "
        ...
        if message.text == 'НОВЫЙ БОКС ДЛЯ САЛАТА — НЕУДОБНЫЙ!':
            bot.send_message(message.from_user.id, "Мы обновили боксы, чтобы блюда, у которых есть соус, было легко "
        ...
        if message.text == 'ПОЧЕМУ ПЛЕНКА ТАК ПЛОХО ОТРЫВАЕТСЯ?':
            bot.send_message(message.from_user.id, "К сожалению, решить проблему с ровным и легким открытием пленки "
        ...
        if message.text == 'ЕСТЬ ЛИ У ВАС НАБОРЫ?':
            bot.send_message(message.from_user.id, "Да, кончено!\n"
        ...
        if message.text == 'МОЖНО ЛИ СДАВАТЬ КОНТЕЙНЕРЫ?':
            bot.send_message(message.from_user.id, "Да, наши контейнеры можно сдавать на переработку, для этого "
        ...
        if message.text == 'МОЖНО ЛИ ЗАМОРАЖИВАТЬ ВАШИ БЛЮДА?':
            bot.send_message(message.from_user.id, "Мы не проводили таких тестирований\n"
                                              "(но клиенты говорят, что можно 🙃)", reply_markup=main_markup)
        if message.text == 'У МЕНЯ ЕСТЬ КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ.':
            bot.send_message(message.from_user.id, "Присылайте ваши коммерческие предложения и идеи по сотрудничеству "
